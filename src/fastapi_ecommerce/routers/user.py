...
SECRET_KEY = '4eaeaa0800e31f170d6eb3b4a3a9444fc0cd41b4044b359bb4289a6fe37c4581'
ALGORITHM = 'HS256'
...
async def create_user(db: Annotated[AsyncSession, Depends(get_db)], create_user: NewUser):
...
async def login(db: Annotated[AsyncSession, Depends(get_db)],
    ...
    token = await create_access_token(username=user.username, user_id=user.id, is_admin=user.is_admin,
                                      is_supplier=user.is_supplier, is_customer=user.is_customer,
